...
export class AuthService {
  ...
  private setSession(authResult: AuthResponse): void {
    ...
    const ttlSec = Number.isFinite(expiresIn) && expiresIn > 0 ? expiresIn : DEFAULT_EXPIRES_IN_SEC;
  ...
  private normalizeAuthResponse(raw: unknown): AuthResponse {
    ...
    const expiresIn = Number(nested['expiresIn'] ?? nested['expires_in'] ?? DEFAULT_EXPIRES_IN_SEC);
    ...
    return {
      ...
      expiresIn: Number.isFinite(expiresIn) && expiresIn > 0 ? expiresIn : DEFAULT_EXPIRES_IN_SEC,
    };
  }
