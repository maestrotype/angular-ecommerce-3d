...
export class BoughtTogetherComponent implements OnInit, OnDestroy {
  ...
  private fetchRecommendations(
    source$: ReturnType<RecommendationsService['getBoughtTogetherProducts']>,
    ...
    source$
      .pipe(
        ...
        catchError(() => {
          ...
          this.notificationService.showError(
            this.translate.instant('PRODUCT.ERRORS.LOAD_BOUGHT_TOGETHER_FAILED'),
            5000,
          );
      ...
      .subscribe({
        next: (products) => {
          this.boughtTogetherProducts = products.map(toProductFromRecommendation);
          this.loading = false;
          this.error = false;
          this.cdr.markForCheck();
