...
export class AdminProductService {
  ...
  upload3dModel(
    file: File,
    apiBase?: string
  ): Observable<{ url: string; publicId: string; localPath?: string }> {
    const formData = new FormData();
  ...
  updateProductOnApi(
    ...
    const headers = this.getHeaders();
    return this.http.patch<Product>(`${apiBase}/products/${id}`, product, {
      headers,
    });
  }

  updateProductOnProduction(
    id: number,
    product: ProductUpdateRequest
  ): Observable<Product> {
    return this.wakeProductionBackend().pipe(
