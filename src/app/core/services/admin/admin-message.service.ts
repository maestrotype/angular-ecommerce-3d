...
export class AdminMessageService {
  ...
  getMessages(): Observable<Message[]> {
    return this.http
      .get<PaginatedResponse<Message>>(this.apiUrl)
      .pipe(map((response) => response.data));
  }

  getMessage(id: number): Observable<Message> {
    return this.http
      .get<ApiResponse<Message>>(`${this.apiUrl}/${id}`)
      .pipe(map((response) => response.data));
  }

  createMessage(message: CreateMessageDto): Observable<Message> {
    return this.http
      .post<ApiResponse<Message>>(this.apiUrl, message)
      .pipe(map((response) => response.data));
  }

  updateMessage(id: number, message: UpdateMessageDto): Observable<Message> {
    return this.http
      .put<ApiResponse<Message>>(`${this.apiUrl}/${id}`, message)
      .pipe(map((response) => response.data));
  }
  ...
  markAsRead(id: number): Observable<Message> {
    return this.http
      .put<ApiResponse<Message>>(`${this.apiUrl}/${id}`, {
        status: 'in_progress',
      })
      .pipe(map((response) => response.data));
  }
  ...
  markAsArchived(id: number): Observable<Message> {
    return this.http
      .put<ApiResponse<Message>>(`${this.apiUrl}/${id}`, { status: 'closed' })
      .pipe(map((response) => response.data));
  }
