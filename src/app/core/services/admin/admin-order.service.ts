...
import { AdminOrder, OrderItem } from '../../../../shared/models/order.model';
import { OrderStats } from '../../../../shared/models/order-stats.model';
...
export class AdminOrderService {
  ...
  getPendingOrdersCount(): Observable<number> {
    return this.http.get<unknown>(`${this.apiUrl}?status=Pending`).pipe(
      map((res) => {
  ...
  updateOrderStatus(
    ...
    return this.http
      .patch<unknown>(`${this.apiUrl}/${id}`, { status, notes })
      .pipe(
        map((res) => {
          const order = unwrapOrderPayload(res);
          if (!order) {
...
function normalizeOrder(raw: Record<string, unknown>): AdminOrder {
  ...
  const items: OrderItem[] = itemsSource.map((item: Record<string, unknown>) => ({
    productId: Number(item?.['productId']) || 0,
    name: String(item?.['name'] || item?.['productName'] || ''),
    price: Number(item?.['price'] ?? item?.['unitPrice']) || 0,
    quantity: Number(item?.['quantity']) || 0,
    image:
      String(item?.['image'] || item?.['imageUrl'] || '').trim() || undefined,
  }));
