...
| G | Premium UI, анимации, полировка | 🔄 In Progress | 4/17 | — |
...
TS `ThemeDefinition` — каталог метаданных, синхронизированный с SCSS `[data-theme]` partials. Legacy per-theme TS files (~600 lines each) удалены.

| ID | Задача | Статус |
|----|--------|--------|
| F1 | Синхронизировать TS-интерфейс `Theme` ↔ SCSS-токены ↔ доки (three-way mirror, ADR-012) | ✅ (2026-08-05) |
| F2 | Мультиизмеренческие темы: typography, spacing, radius, elevation, density, motion (ADR-004) | ✅ (2026-08-05) |
| F3 | Валидация тем: runtime-проверка наличия обязательных CSS-переменных | ✅ (2026-08-05) |
