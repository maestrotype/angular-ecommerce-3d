...
| B | Material Overrides Centralization | 🔄 In Progress | B1+B2 done: dump migrated to `overrides/_material-overrides.scss` on semantic/bridge tokens; B3–B4 remaining |
...
- [ ] Material overrides: dump migrated to `overrides/_material-overrides.scss` (B2); component `.mat-` + `material-theme.scss` remain (B3–B4)
